      ...
      CSSwhat(rule.selector).forEach(function(select) {
        select.forEach(function(token) {
          ...
          if (token.type === 'attribute' && token.name === 'class') {
            ...
            keyLookup = ['class', token.value].join(LOOKUP_DELIMITER);
          } else if (token.type === 'attribute' && token.name === 'id') {
            // Handle #something and id=something
            keyLookup = ['id', token.value].join(LOOKUP_DELIMITER);
          } else if (token.type === 'attribute' && token.name === 'for') {
          ...
          if (pointer) {
            newSelector = newSelector.replace(token.value, pointer);
          }
        });
      });
